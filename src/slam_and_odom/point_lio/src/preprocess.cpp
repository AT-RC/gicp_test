...
void Preprocess::rsairy_handler(const sensor_msgs::msg::PointCloud2::SharedPtr & msg)
{
  ...
  double t_last = pl_orig.points[plsize - 1].timestamp;
  double span_s = t_last - t0;
  ...
  for (int i = 0; i < plsize; i++) {
    ...
    // 鲁棒性处理：如果驱动没给有效时间戳，则按点数均匀分配（100ms一帧）
    if (span_s > 0.001) {
      added_pt.curvature = (pl_orig.points[i].timestamp - t0) * 1000.0;
    } else {
      added_pt.curvature = (float)i / (float)plsize * 100.0;
    }
  ...
  deque<double> & time_lidar, const int required_frame_num, int scan_count)
{
  ...
  if (lidar_type == VELO16) {
    ...
    for (int i = 0; i < plsize; i++) {
      ...
      added_pt.z = pl_orig.points[i].z;
      added_pt.intensity = pl_orig.points[i].intensity;
      added_pt.curvature = (pl_orig.points[i].timestamp - t0) * 1000.0;  // s to ms
